...
const WhyUsSection = ({ onBack }: { onBack: () => void }) => {
  const { t } = useLanguage();
    ...
    <SectionPanel backgroundImage={sectionBg} onBack={onBack}>
      <div className="max-w-6xl mx-auto flex flex-col justify-center min-h-[60vh]">
        <motion.h1
          ...
          className="text-4xl md:text-6xl font-heading font-bold tracking-tight text-foreground mb-16"
        >
          {t('whyUs')}
        </motion.h1>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          {pillars.map((p, i) => (
            ...
            >
              <h2 className="text-5xl md:text-7xl font-heading font-black text-gradient-gold mb-4">
                {t(p.key)}
              </h2>
